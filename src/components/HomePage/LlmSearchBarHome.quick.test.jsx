/**
 * Home hides Quick without changing the chat composer's saved preference.
 * A hidden control must not silently enable Quick for a new home submission.
 */
...
it('hides Quick and does not apply a saved chat-page Quick preference on home', async () => {
    window.localStorage.setItem('glkb_chat_effort', 'quick');
    setup();
    await waitFor(() => expect(screen.getByRole('button', { name: /^Model:/, hidden: true })).toBeEnabled());
